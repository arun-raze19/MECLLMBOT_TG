    ...
    async def wrapped(update, context, *args, **kwargs):
        ...
        if user_id != int(os.getenv("AUTHORIZED_USER")):
            ...
            await update.message.reply_animation(
                "MEC LLM BOT",
                caption="this is a llm bot created by mailam engineering students",
            )
            return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        text="Hi. It's MEC LLM  Bot. You can ask me anything and talk to me about what you want",
...
async def reply_and_new_message(
    ...
    """Send user message to MECLLMBOT core and respond and wait for new message or exit command"""
    query = update.callback_query
...
async def generate_text_from_image(
    ...
    try:
        ...
        if not response:
            raise Exception("Empty response from MECLLMBOT")
    except Exception as e:
